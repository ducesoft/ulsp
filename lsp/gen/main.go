...
`)
	...
	out.WriteString("type AbsServer struct {\n}\n")
	for _, k := range sdecls.keys() {
		decl := strings.TrimSpace(strings.Split(sdecls[k], "//")[0])
		out.WriteString("func (that *AbsServer) ")
		out.WriteString(decl)
		out.WriteString(" {\n")
		if strings.LastIndex(decl, "error") == len(decl)-5 {
			out.WriteString("return ErrMethodNotFound")
		} else {
			out.WriteString("return nil, ErrMethodNotFound")
		}
		out.WriteString("}\n\n")
	}
