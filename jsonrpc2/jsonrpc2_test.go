...
type streamMaker func(conn io.ReadWriteCloser) ObjectStream
...
func TestClientServer(t *testing.T) {
	t.Run("tcp-varint-object-codec", func(t *testing.T) {
		testClientServerForCodec(t, func(conn io.ReadWriteCloser) ObjectStream {
			return NewBufferedStream(conn, VarintObjectCodec{})
		})
	})
	t.Run("tcp-vscode-object-codec", func(t *testing.T) {
		testClientServerForCodec(t, func(conn io.ReadWriteCloser) ObjectStream {
			return NewBufferedStream(conn, VSCodeObjectCodec{})
		})
	})
	t.Run("tcp-plain-object-codec", func(t *testing.T) {
		testClientServerForCodec(t, func(conn io.ReadWriteCloser) ObjectStream {
			return NewBufferedStream(conn, PlainObjectCodec{})
		})
	})
	...
	t.Run("websocket", func(t *testing.T) {
		...
		s := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
			...
			defer c.Close()
			jc := NewConn(r.Context(), NewObjectStream(c), &ha)
			<-jc.DisconnectNotify()
			close(done)
		}))
		defer s.Close()
		...
		defer c.Close()
		testClientServer(ctx, t, NewObjectStream(c))

		<-done // keep the test running until the WebSocket disconnects (to avoid missing errors)
	})
}

func testClientServer(ctx context.Context, t *testing.T, stream ObjectStream) {
	hb := testHandlerB{t: t}
	cc := NewConn(ctx, stream, &hb)
	defer func() {
...
type handlerFunc func(context.Context, *Conn, *Request)

func (h handlerFunc) Handle(ctx context.Context, conn *Conn, req *Request) {
	h(ctx, conn, req)
}
...
func serve(ctx context.Context, lis net.Listener, h Handler, streamMaker streamMaker, opts ...ConnOpt) error {
	for {
		conn, err := lis.Accept()
		if err != nil {
			return err
		}
		NewConn(ctx, streamMaker(conn), h, opts...)
	}
}
