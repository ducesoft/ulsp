...
func (c *Conn) Close() error {
	return c.close(context.Background(), nil)
}
...
func (c *Conn) close(ctx context.Context, cause error) error {
	c.mu.Lock()
	defer c.mu.Unlock()
	...
	for _, caller := range c.pending {
		close(caller.done)
	}

	if cause != nil && cause != io.EOF && cause != io.ErrUnexpectedEOF {
		log.Error(ctx, "jsonrpc2: protocol error: %v\n", cause)
	}
...
func (c *Conn) readMessages(ctx context.Context) {
	...
	for err == nil {
		...
		case m.response != nil:
			...
			if resp != nil {
				...
				switch {
				case caller == nil:
					log.Info(ctx, "jsonrpc2: ignoring response #%s with no corresponding request\n", id)

				case resp.Error != nil:
					caller.done <- resp.Error
					close(caller.done)
	...
	if err = c.close(ctx, err); nil != err {
